...
public class ForgeHelpCommand<M> extends AbstractForgeCommand<M> {
    ...
    public ForgeHelpCommand(M modInstance, ICommandSender sender, AbstractForgeCommand<M> mainCommand) {
        super(modInstance, "help", "Display help info for " + mainCommand.getUsage(sender), ForgeCommandUsage.of(new ForgeCommandArgument(TextFormatting.getTextWithoutFormattingCodes(mainCommand.getUsage(sender))), new ForgeCommandArgument("help")));
        this.mainCommand = mainCommand;
    }

    @Override
    protected void build() {
        setExecutor((server, sender, args) -> {
            ...
            mainCommand.getCommands().values().forEach(command -> sender.sendMessage(new TextComponentString(command.getCommandHelpInfo(sender))));
        });
    }
}
