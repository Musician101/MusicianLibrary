...
public class SpongeBookGUI extends BookGUI<PluginContainer, ServerPlayer, Component> {
    ...
    public SpongeBookGUI(@Nonnull PluginContainer plugin, @Nonnull ServerPlayer player, @Nonnull ItemStack book, @Nonnull BiConsumer<ServerPlayer, List<Component>> action) {
        super(plugin, player, player.inventory().hotbar().selectedSlotIndex(), action);
        book.offer(getBookUUIDKey(plugin), player.uniqueId());
        player.inventory().hotbar().set(bookSlot, book);
    ...
    public static boolean isEditing(@Nonnull PluginContainer plugin, @Nonnull ServerPlayer player) {
        return player.inventory().slots().stream().map(Inventory::peek).anyMatch(itemStack -> itemStack.get(getBookUUIDKey(plugin)).filter(uuid -> uuid.equals(player.uniqueId())).isPresent());
    }

    @Listener
    public void clickBook(ClickContainerEvent event, @Getter("cursorTransaction") Transaction<ItemStackSnapshot> transaction, @First ServerPlayer player) {
        event.setCancelled(isEditing(plugin, player) && transaction.finalReplacement().createStack().get(getBookUUIDKey(plugin)).isPresent());
    }
    ...
    protected void remove() {
        player.inventory().hotbar().set(bookSlot, ItemStack.empty());
